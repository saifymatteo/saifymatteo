...
- **Tag → environment mapping**: one workflow listens on `release/nextjs/v*`; stable tags (`vX.Y.Z`) deploy production, prerelease tags (`vX.Y.Z-beta.N`) deploy Staging. No local deploy script and no manual workflow dispatch exist — tags are the only Deploy path.
