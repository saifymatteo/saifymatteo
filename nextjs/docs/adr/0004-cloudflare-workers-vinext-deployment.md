...
Accepted trade-offs: `next/image` renders local images as `<img>` + responsive `srcSet` without optimization or resizing (the paid Images binding is the future unlock, not a default); `next/font/google` loads from the Google CDN at runtime instead of build-time self-hosting; vinext's gaps ("with caution") are mitigated by `vinext check` gating the migration, the non-destructive dual toolchain, and the fallback option of Netlify/OpenNext being one decision away if a blocker appears. The tag becomes the source of truth for the deployed version; `package.json` bumps are an optional chore commit at release time. Rollback of the migration itself is trivially cheap since the Next toolchain remains installed alongside vinext.
